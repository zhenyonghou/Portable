...
- (instancetype)initWithName:(NSString *)name
{
    ...
    if (self = [super init]) {
        _name = [name copy];
        _queue = [BBDiskCache sharedQueue];
        ...
        __weak BBDiskCache *weakSelf = self;

        dispatch_async(_queue, ^{
            BBDiskCache *strongSelf = weakSelf;
...
+(BOOL)moveItemAtURLToTrash:(NSURL *)itemURL
{
    ...
    NSURL *uniqueTrashURL = [[BBDiskCache sharedTrashURL] URLByAppendingPathComponent:uniqueString];
...
- (BOOL)removeFileAndExecuteBlocksForKey:(NSString *)key
{
    ...
    BOOL trashed = [BBDiskCache moveItemAtURLToTrash:fileURL];
    if (!trashed)
        return NO;
    
    [BBDiskCache emptyTrash];
...
- (void)removeAllObjects:(BADiskCacheBlock)block
{
    ...
    dispatch_async(_queue, ^{
        ...
        [BBDiskCache moveItemAtURLToTrash:strongSelf->_cacheURL];
        [BBDiskCache emptyTrash];
...
- (id <NSCoding>)objectForKey:(NSString *)key
{
    ...
    [self objectForKey:key block:^(BBDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        objectForKey = object;
...
- (NSURL *)fileURLForKey:(NSString *)key
{
    ...
    [self fileURLForKey:key block:^(BBDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key
{
    ...
    [self setObject:object forKey:key block:^(BBDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    [self removeObjectForKey:key block:^(BBDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)renameObjectWithOldKey:(NSString *)oldKey newKey:(NSString *)newKey
{
    ...
    [self renameObjectWithOldKey:oldKey newKey:newKey block:^(BBDiskCache *cache, NSString *key, id<NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSize:(NSUInteger)byteCount
{
    ...
    [self trimToSize:byteCount block:^(BBDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToDate:(NSDate *)date
{
    ...
    [self trimToDate:date block:^(BBDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSizeByDate:(NSUInteger)byteCount
{
    ...
    [self trimToSizeByDate:byteCount block:^(BBDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeAllObjects
{
    ...
    [self removeAllObjects:^(BBDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)enumerateObjectsWithBlock:(BADiskCacheObjectBlock)block
{
    ...
    [self enumerateObjectsWithBlock:block completionBlock:^(BBDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
