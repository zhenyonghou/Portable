/**
 `BADiskCache` is a thread safe key/value store backed by the file system. It accepts any object conforming
 ...
 The designated initializer for `BADiskCache` is <initWithName:>. The <name> string is used to create a directory
 ...
 Because this cache is bound by disk I/O it can be much slower than <BAMemoryCache>, although values stored in
 `BADiskCache` persist after application relaunch. Using <BACache> is recommended over using `BADiskCache`
...
typedef void (^BADiskCacheBlock)(BADiskCache *cache);
typedef void (^BADiskCacheObjectBlock)(BADiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL);

@interface BADiskCache : NSObject
...
/**
 The URL of the directory used by this cache, usually `Library/Caches/com.tumblr.BADiskCache.(name)`
...
/**
 ...
 For example:
    ...
    dispatch_sync([BADiskCache sharedQueue], ^{
        NSLog(@"accurate, unchanging byte count: %d", [[BADiskCache sharedCache] byteCount]);
    });
 */
 ...
 */
@property (copy) BADiskCacheObjectBlock willAddObjectBlock;

/**
 A block to be executed just before an object is removed from the cache. The queue waits during execution.
 */
@property (copy) BADiskCacheObjectBlock willRemoveObjectBlock;
...
/**
 重命名缓存文件，以下情况有可能会用到该方法：
 1. 录制音频，录制完成后，temp文件需要重命名
 2. 上传文件时候，服务器重新命名了该文件.
 3. 大文件操作
 */
 ...
 */
- (void)trimToDate:(NSDate *)date block:(BADiskCacheBlock)block;
...
/**
 ...
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
- (void)trimToSize:(NSUInteger)byteCount block:(BADiskCacheBlock)block;
...
/**
 ...
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
- (void)trimToSizeByDate:(NSUInteger)byteCount block:(BADiskCacheBlock)block;
