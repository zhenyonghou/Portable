...
@implementation BBFileDiskCache
...
- (id)initWithName:(NSString *)name
{
    ...
    if (self = [super init]) {
        _name = name;
        _queue = [BBFileDiskCache sharedQueue];
...
//        _didRemoveAllObjectsBlock = nil;
        ...
        __weak BBFileDiskCache *weakSelf = self;
        
        dispatch_async(_queue, ^{
            BBFileDiskCache *strongSelf = weakSelf;
...
//        _willRemoveObjectBlock(self, key, nil, fileURL);
    ...
    BOOL remove = [BBFileDiskCache removeItemFromDiskAtURL:fileURL];
    if (!remove)
        return NO;
...
//            strongSelf->_willRemoveAllObjectsBlock(strongSelf);
        ...
        [BBFileDiskCache removeItemFromDiskAtURL:strongSelf->_cacheURL];
...
- (NSURL *)fileURLForKey:(NSString *)key
{
    ...
    [self fileURLForKey:key block:^(BBFileDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
...
- (void)addFileForKey:(NSString*)key
{
    ...
    [self addFileForKey:key block:^(BBFileDiskCache *cache, NSString *key, id<NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeFileForKey:(NSString *)key
{
    ...
    [self removeFileForKey:key block:^(BBFileDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)renameFileWithOldKey:(NSString *)oldKey newKey:(NSString *)newKey
{
    ...
    [self renameFileWithOldKey:oldKey newKey:newKey block:^(BBFileDiskCache *cache, NSString *key, id<NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)updateForKey:(NSString*)key
{
    ...
    [self updateForKey:key block:^(BBFileDiskCache *cache, NSString *key, id<NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
